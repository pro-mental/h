  ...
  <v-container class="align-content-space-between" fluid fill-height>
    ...
    <v-container class="px-auto py-0" fluid>
      ...
      <v-row class="mt-2">
        <!--Opcoes de Formuario-->
        <v-col cols="12">
          <v-item-group v-if="steps === null" active-class="justify-space-around" v-model="cardSelection">
            <v-row class="justify-space-around">
              <CardItemGender v-for="(cardInfo, index) in cardGender" :key="index" :colorCard="cardInfo.cardColor" :gender="cardInfo.label" :imageSource="cardInfo.imageSource" />
            </v-row>
          </v-item-group>

          <v-radio-group v-else-if="steps === 'genderOk'" v-model="radio" row required>
    ...
    <v-container>
      <v-row class="justify-center">
        <v-col class="col-12 col-sm-8">
          <v-btn v-if="cardSelection !== null && steps === null" block color="primary" dark large rounded @click="confirmar('genderOk')"> confirmar </v-btn>

          <v-btn v-else-if="radio !== null && steps === 'genderOk'" color="primary" block dark large rounded @click="confirmar('workOk')"> confirmar </v-btn>

          <v-btn v-else-if="steps === 'workOk' && ageField !== ''" color="primary" block dark large rounded @click="confirmar('ageOk')"> confirmar </v-btn>
        </v-col>
      </v-row>
    </v-container>
  </v-container>
